...
<<<
== Aufwand
Der Aufwand betrug aufgrund Vorerfahrung ~20h.
...
Der API-Endpunkt kann in den environment Files unter `apiBaseUrl` geändert werden.
...
Darunter wird eine Angular-Materials Sidebar-Komponente angezeigt, wo die entsprechenden Pages ausgewählt werden können.
Dies ist aber nur möglich, wenn man auch angemeldet ist.

.Übersicht
image::{img}/overview.png[pdfwidth=65%]

<<<
== Authentifizierung

Hier wurde kein eigener Keycloak-Server verwendet, sondern einfach der Identity-Server von Manfred Steyr.
...
image::{img}/metrics_overview.png[pdfwidth=70%]
...
image::{img}/detectors.png[pdfwidth=75%]

Detaillierte Informationen werden angezeigt, wenn man einen Detektor auswählt.

.Detailansicht eines Detektors
image::{img}/detector_details.png[pdfwidth=65%]
...
Für die Clients gibt es auch eine Übersicht, wobei hier App-Key und Instanzen angezeigt werden.
Es können neue Clients hinzugefügt werden und der App-Key kopiert werden.
Dies erfolgt wieder über den Floating-Action-Button rechts unten.

.Erstellen neuer Clients
image::{img}/add_client.png[pdfwidth=65%]

Nach dem Drücken auf "Generate App-Key" wird ein neuer Client erstellt, der App-Key kann danach kopiert werden.

.Erfolgreiche Erstellung
image::{img}/add_client_app_key.png[pdfwidth=65%]